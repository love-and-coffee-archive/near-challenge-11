...
const CONTRACT_NAME = "dev-1638529663333-98526917092338" || "tic-tac-toe.testnet";
...
async function initContract() {
  ...
  window.contract = await new _nearApiJs.Contract(window.walletConnection.account(), nearConfig.contractName, {
    ...
    viewMethods: ["viewBoard"],
    // Change methods can modify the state. But you don't receive the returned value when called.
    changeMethods: ["markCell", "startNewGame"],
...
startNewGameElement.onclick = async () => {
  try {
    ...
    const response = await window.contract.startNewGame({}, BOATLOAD_OF_GAS);
...
function updateResponse(message, success) {
  ...
  responseContainer.innerHTML = "<span>" + message + "</span>";
  setTimeout(() => {
